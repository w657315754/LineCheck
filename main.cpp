...
//
//	//[ xmin, ymin, xmax, ymax, pointX, pointY, mask, row_s, col_s, number, L_tmp] = Init( NumLine,img0,img,PosLine, dx, dy,d,InitNum );
//
//	mwArray xmin, ymin, xmax, ymax, _pointX, _pointY, mask, row_s, col_s, number, L_tmp;
//
//	mwArray Ratio,flag,x,y;
//
//	auto NumLine = mwArray(1, 1, mxINT32_CLASS);
//	auto img0 = cvtCvimgToMWArray(first);
//	auto img = cvtCvimgToMWArray(first);
//	auto PosLine = mwArray(lines*3, 2, mxINT32_CLASS);
//	auto dx = mwArray(1, 1, mxDOUBLE_CLASS);
//	auto dy = mwArray(1, 1, mxDOUBLE_CLASS);
//	auto d = mwArray(1, 1, mxDOUBLE_CLASS);
//	auto InitNum = mwArray(1, 1, mxDOUBLE_CLASS);
//
//	for(int i=0;i<lines*3;i++){
//		PosLine(i + 1, 1) = pointX[i];
//		PosLine(i + 1, 2) = pointY[i];
//	}
//	cout<<"线位置:"<<endl
//		<<PosLine<<endl;
//	cout<<"线数量："<<lines<<endl;
...
//	InitNum(1, 1) = 10;
//
//	Init(11,xmin, ymin, xmax, ymax, _pointX, _pointY, mask, row_s, col_s, number, L_tmp,NumLine,img0,img,PosLine, dx, dy,d,InitNum);
//
//
...
//	GetBorder(3,tmp_img0, xLimit, yLimit,PosLine,NumLine,img0,d1,d2);
//	//int lines = 3;
//	int count = 0;
//	while (true) {
//		//video >> current;
...
//		detLine(4, rS, rN, rX, rY, img,NumLine,PosLine, draw,tmp_img0, xLimit, mth, img0,dx,dy,d,flag_med,alpha);
//		//[ number,flag] = Detect(img, img0, xmin, ymin, xmax, ymax, mask,InitNum,number,L_tmp, pointX, pointY);
//		Detect(5,Ratio,number,flag,x,y,img, img0, xmin, ymin, xmax, ymax, mask,InitNum,number,L_tmp, _pointX, _pointY,x,y);
//
//		//cout<<"VS输出flag："<<((int)flag==1)<<endl;
//		//if((int)flag!=1){//线掉落
//		//	//cout<<"VS输出 线掉落"<<endl;
//		//	status.push(1);
//		//	sum_exception +=1;
//
//		//}
//		//if((int)flag==1){//到最后一根线还没break，说明都正常
//		//	//cout<<"VS输出 线正常"<<endl;
...
//				break;
//			}
//			if((i == lines - 1)&&(int)flag==1){//到最后一根线还没break，说明都正常
//				status.push(0);//正常状态为0，异常状态为1
//			}
//		}
...
//
//	radio_th = config["radio_th"].asDouble();//用于新算法调节灵敏度
//}
//
//
//
